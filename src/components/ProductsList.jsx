...
export default function ProductsList() {
  ...
  const storeDomain = useSelector(
    (store) => store.customerStore?.storeDomainResource
  );
  ...
  const sorthandler = (sortBy) => {
    if (sortBy == "productName") {
      sortedProducts = [...productsList].sort((a, b) =>
      ...
      <div className="m-16  flex flex-col bg-white shadow-lg px-5 text-black">
        ...
        <div className="justify-between items-center p-2 border-b-2 bg-gray-300 rounded ">
          <div className="flex items-center font-semibold self-center align-middle content-center">
            ...
            <span
              className="w-1/5 "
              onClick={() => sorthandler("productStockQuantity")}
            >
              Stock
            </span>
        ...
        {!sortedProducts ? (
          <h1 className="text-2xl text-center p-3 text-gray-800 font-semibold">
            Please add products <Link to="/add-product">Here</Link>{" "}
          </h1>
        ) : (
          <>
